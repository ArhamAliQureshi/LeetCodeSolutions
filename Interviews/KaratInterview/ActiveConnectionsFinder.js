...
console.log(main(data, 3));


function main(events, target){
  let resultObj = parseData(events)   
  return findConnection(resultObj, target)
}
...
function parseData(events){
  ...
  events.map(item=>{
    if(item[0]==='CONNECT'){
      ...
      resultObj[item[1]].push(item[2])
      resultObj[item[2]].push(item[1])
    }
    else{
      let index1 = resultObj[item[1]].indexOf(item[2]);
      let index2 = resultObj[item[2]].indexOf(item[1]);
     
      resultObj[item[1]].splice(index1, 1);
      resultObj[item[2]].splice(index2, 1); 
...
function findConnection(obj, targetNumber){
  let keys = Object.keys(obj)
  let lessConnections = [];
  let moreConnections = [];
  keys.map(item=>{    
    if(obj[item].length>=targetNumber){
      moreConnections.push(item);
    }
    else{
      lessConnections.push(item)
    }
  })
  
  return {lessConnections , moreConnections}
}
